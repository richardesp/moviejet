...
CREATE
OR REPLACE TRIGGER trigger_auditoria BEFORE INSERT OR DELETE
OR
UPDATE
...
INTO stock
FROM Pelicula
WHERE titulo = :new.titulo_pelicula;

dbms_output
.
put_line
(USER || ' está insertando' ||
...
INTO stock
FROM Pelicula
WHERE titulo = :old.titulo_pelicula;
...
CREATE
OR REPLACE TRIGGER trigger_fecha_alquiler BEFORE INSERT OR
UPDATE
    ON ALQUILER FOR EACH ROW
BEGIN
    IF
:new.alquiler_fin < SYSDATE THEN
        RAISE_APPLICATION_ERROR(-20002, 'La fecha de alquiler no puede ser menor a la fecha actual');
    ELSIF
:new.alquiler_fin < :new.alquiler_inicio + 1 THEN
...
CREATE
OR REPLACE TRIGGER trigger_valoracion_pelicula BEFORE INSERT OR
UPDATE
    ON Valoracion FOR EACH ROW
BEGIN
    IF
:new.puntuacion < 0 OR :new.puntuacion > 10 THEN
...
CREATE
OR REPLACE TRIGGER trigger_alquiler_usuario BEFORE INSERT OR
UPDATE
    ON ALQUILER FOR EACH ROW
DECLARE
...
CREATE
OR REPLACE TRIGGER trigger_stock_alquiler BEFORE INSERT
    ON Alquiler FOR EACH ROW
DECLARE
stock INTEGER;
BEGIN
